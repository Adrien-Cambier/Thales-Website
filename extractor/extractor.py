...
class Extractor:
    ...
    def run(self) -> bool:
        ...
        # And adds them to the SQL db.
        #
        print(termcolor.colored("[+]", "yellow"), f"Running...")

        if not self.extract_frames_from_file_and_add_them_to_db():
            print(termcolor.colored("[-]", "red"), "Error")
            return False
...
notes:
  [--full] and [--short] can't be used together.""".format(sys.argv[0]))
    ...
    def extract_frames_from_file_and_add_them_to_db(self) -> bool:
        """
        ...
        try:
            ...
            while bit_stream.pos < len(bit_stream):
                sys.stdout.write("\r" + termcolor.colored("[*]", "blue") + f" {self.frame_counter + 1}")
                sys.stdout.flush()
                ...
                #
                # Removes the "0x" from hexa values
                #
                for field in self.current_frame.fields.items():
                    field_name = field[0]
                    field_value = field[1]["value"]


                    if field_value.startswith("0x"):
                        field[1]["value"] = field_value[2:]

                        self.current_frame.fields[field_name] = field[1]

                #
                # Inserts the current frame in the SQL db.
                #
                if not self.insert_frames_into_db(self.current_frame):
                    return False
    ...
    def insert_frames_into_db(self, frame) -> bool:
        """
    ...
    def calculate_msg_type(self):
        ...
        self.current_frame.message_type = self.current_frame.fields["field_14"]["value"][2:] + \
